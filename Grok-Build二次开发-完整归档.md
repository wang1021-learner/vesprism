...
**当时的官方改动**：把这个函数从 `fn` 改成 `pub fn`（一行改动）。  
早期遵循「最小侵入」以降低合入成本；**自 2026-07-29 起，有更好方案时允许更大范围改官方**，见 `docs/官方代码修改原则.md`。
...
> **现行规则：** [`docs/官方代码修改原则.md`](./docs/官方代码修改原则.md)  
> 下文第 1 条已按 2026-07-29 政策更新；历史章节中「只加不改」表述作废。

1. **改造原则（现行）**：默认可在桌面壳 / 自建 crate 完成；**有非常好的方案时可以改官方代码**（正确性、安全、产品能力、长期可维护性）。  
   改官方须可描述、尽量局部、可验证、可回灌上游合并。早期「只加不改」是降冲突策略，**不再是禁令**。
