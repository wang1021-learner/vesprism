...
> **官方代码规则（现行）：** 详见 [`docs/官方代码修改原则.md`](./docs/官方代码修改原则.md)。  
> 旧「只加不改、禁止改官方」**已废止**。默认可在壳层完成；**有明显更好的方案时，允许改官方**。

1. **官方代码可改，但要够好**：优先复用官方能力与自建 crate；根因在官方、收益清晰、可合并可验证时，应改 `codegen` / `xai-grok-*`，不必为了零 diff 硬绕。  
...
7. **系统提示词等认知层配置，优先官方覆盖机制**；机制不够时再扩展官方入口，而非 GUI 永久 hack。  
8. 上游仓库不接受 PR，只能自行 fork 维护，定期 `git fetch upstream` 手动同步；官方补丁需可回灌。
