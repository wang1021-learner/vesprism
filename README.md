...
同一个窗口里三套产品：**编码**对着 Git 仓库改代码，**工作台**用自然语言编排多 Agent 流程，**写完**按章循环写长篇。  
...
三套产品共用同一套模型和密钥，主界面和会话列表按壳分开。工作台的流程落到当前仓库；编码对话可以调用已经发布的流程。写完不和编码工作区抢 cwd。
...
画布 / 编制用过的会话默认不进普通历史；只有保存过产物的才会出现在侧栏「工作台」。未绑定产物的画布只活在当前 Tab，这是设计，不是丢失。写完侧栏是书，不是引擎会话。
