...
同一个窗口里四套产品：**编码**对着 Git 仓库改代码，**工作台**用自然语言编排多 Agent 流程，**写完**按章循环写长篇，**办公**对着材料夹交稿（demo）。  
...
- **长篇按章写完。** 写完用章循环 + 窄切片 + 入卷案卷，不靠一次读完全书；书库、检查、正史导出都在桌面里。
...
| 能力 | 说明 |
|------|------|
| 书库 | 只读 meta，不灌每章正文；打开一本才组装 |
| 章循环 | 章纲、切块、试笔、检查、入卷；摘要和伏笔是硬门 |
| 进度 | 已入卷字数、距百万、本卷写到哪、章表（空/试笔/已检/已入卷） |
| 导出 | 只出已进正史的 txt；可按章或按卷 |
| 隔离 | 写台会话锁只答，不准动文件；书在 `~/.vesprism/books/` |

### 办公（demo）

| 能力 | 说明 |
|------|------|
| 办公桌 | Cowork 式任务台：起手、计划、产物预览 |
| 材料夹 | 演示夹「本周材料」，不是 Git 仓库 |
| 交稿 | 周报 Word / 汇报 PPT / 合同要点；还不接引擎 |

四套产品共用同一套模型和密钥，主界面和会话列表按壳分开。工作台的流程落到当前仓库；编码对话可以调用已经发布的流程。写完、办公都不和编码工作区抢 cwd。
...
| **写完** | 按章把长篇写完、交出去 | 书库、章表（空/试笔/已检/已入卷）、稿纸与入卷、正史 txt 导出 |
| **办公** | 对着材料夹把稿交出去（demo） | 办公桌：起手（周报 / PPT / 合同要点）、计划、产物预览。还不接引擎 |

画布 / 编制用过的会话默认不进普通历史；只有保存过产物的才会出现在侧栏「工作台」。未绑定产物的画布只活在当前 Tab，这是设计，不是丢失。写完侧栏是书，办公侧栏是任务，都不是编码会话。
...
| `~/.vesprism/books/` | 写完：每书 `meta.json` + `book.json` + `chapters/` |
| `~/.vesprism/writing/` | 写完每书会话工作目录 |
| 当前打开的仓库 | 编码改动；工作台发布的流程 sidecar 在 `~/.vesprism/workflows/` |
