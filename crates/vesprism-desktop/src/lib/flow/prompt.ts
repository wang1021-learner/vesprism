...
- agent  Agent：挂一份组装单。params: {label?, presetId?, role?, model?, prompt?}。model 可空（继承会话）。
- tool   工具：调用工具或运行命令。params: {label?, toolName?, command?, args?}。
- flow   子流程：引用另一个已发布流程。params: {label?, flowId?, input?}。发布时会把对方图画进本流程，包内不再调用 flow__。
- branch 分支：按上一节点输出分流。params: {label?, condition: "success"|"failure"|"expression", expression?}。
...
4. 每个 branch 节点必须至少有一条出边；成功/失败请用 label "success" / "failure"。
