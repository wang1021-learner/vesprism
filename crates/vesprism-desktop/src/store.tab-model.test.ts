...
describe('Tab 活动灯', () => {
  ...
  it('findTabByUtilityKind 可复用 office-desk 专用 Tab', () => {
    createTab('tab-office', { utilityKind: 'office-desk', chatTitle: '办公桌' })
    expect(findTabByUtilityKind('office-desk')).toBe('tab-office')
    expect(findTabByUtilityKind('writing-desk')).toBeUndefined()
    expect(isWorkbenchUtility('office-desk')).toBe(true)
  })
  ...
  it('upsertSubagent 会 bump 版本，试跑详情才能看到新 spawn', () => {
    createTab('tab-flow', { utilityKind: 'flow-canvas' })
    const before = $subagentRevision.get()
    upsertSubagent('tab-flow', {
      subagentId: 'x',
      childSessionId: 'sess-x',
      parentSessionId: 'p',
      description: '摘要',
    })
    expect($subagentRevision.get()).toBeGreaterThan(before)
    expect(getTabState('tab-flow')?.subagents.map((s) => s.childSessionId)).toEqual(['sess-x'])
  })
